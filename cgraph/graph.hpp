...
public:
    ...
    void addEdge(i32 from, i32 to, f32 weight)
    {
        assert(!empty());
    ...
    // Graph Ops
    std::vector<f64> getAllShortestPath()
    {
#ifdef _OPENMP
...
#endif
        // Use Floyd Algorithm
        std::vector<f64> dist(num_nodes * num_nodes, std::numeric_limits<f64>::max());
        // clang-format off
        ...
        for (i32 i = 0; i < num_nodes; i++)
        {
            ...
            for (const auto &e : edges_per_node[i])
                dist[arrIdx(i, e.to)] = std::min(dist[arrIdx(i, e.to)], (f64)e.weight);
        }
        for (i32 k = 0; k < num_nodes; k++)
#pragma omp parallel for schedule(dynamic)
            for (i32 i = 0; i < num_nodes; i++)
                if (dist[arrIdx(i, k)] != std::numeric_limits<f64>::max())
                    for (i32 j = 0; j < num_nodes; j++)
                        if (dist[arrIdx(k, j)] != std::numeric_limits<f64>::max())
                        {
    ...
    std::vector<f64> getShortestPathCond(i32 src)
    {
        assert(!empty());
        ...
        std::vector<f64> dist(
            num_nodes, std::numeric_limits<f64>::max());
        std::vector<bool> visited(num_nodes, false);
        std::priority_queue<std::pair<f64, i32>> pq;
    ...
    std::vector<f64> getShortestPath(i32 src)
    {
        assert(!empty());
        assert(0 <= src && src < num_nodes);
